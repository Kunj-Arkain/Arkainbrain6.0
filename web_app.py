...
def job_files(job_id):
    ...
    variants_btn = ""
    parent_for_variants = job.get("parent_job_id") or job_id
    db_v = get_db()
    ...
    <p style="color:var(--text-muted);font-size:12px;margin-bottom:4px">{len(files)} files generated · v{job.get("version") or 1}</p>
    {version_html}
...
def job_iterate(job_id):
    ...
    root_id = job.get("parent_job_id") or job_id
    db2 = get_db()
    current_version = job.get("version") or 1
...
def job_diff(job_id, other_id):
    ...
    va = job_a.get("version") or 1; vb = job_b.get("version") or 1
...
def job_variants(job_id):
    ...
    for v in variants:
        ...
        variant_data.append({"id":v["id"],"status":v["status"],"label":vc.get("label",f"V{v.get('version','?')}"),"strategy":vc.get("strategy",""),"metrics":m})
